export function generateInsights(metrics) {
  ...
  let summary = `Your store generated $${totalRevenue.toLocaleString()} in revenue from ${totalOrders.toLocaleString()} orders`;
  
  if (topProduct) {
    summary += `, with "${topProduct.name}" being your top performer at $${topProduct.revenue.toLocaleString()}.`;
  } else {
    summary += '.';
  }
  ...
  if (avgProfitMargin > 0) {
    insights.push(
      `Overall profit margin averages ${avgProfitMargin.toFixed(1)}%, ${avgProfitMargin > 20 ? 'showing strong profitability' : 'consider optimizing pricing strategy'}.`
    );
  }
